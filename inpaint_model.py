...
class CoarseNet(nn.Module):
    ...
    def debug(self):
        print('CoarseNet Pamameters------------------------------------')
        for name, param in self.named_parameters():
            print('%s\t: %d ' % (name, int(param.isnan().any())))
        """
        print('conv1  : %d ' % (sum(self.conv1.debug_val().is_nan())))
        print('conv2  : %d ' % (sum(self.conv2.debug_val().is_nan())))
        print('conv3  : %d ' % (sum(self.conv3.debug_val().is_nan())))
        print('conv4  : %d ' % (sum(self.conv4.debug_val().is_nan())))
        print('conv5  : %d ' % (sum(self.conv5.debug_val().is_nan())))
        print('conv6  : %d ' % (sum(self.conv6.debug_val().is_nan())))
        print('conv7  : %d ' % (sum(self.conv7.debug_val().is_nan())))
        print('conv8  : %d ' % (sum(self.conv8.debug_val().is_nan())))
        print('conv9  : %d ' % (sum(self.conv9.debug_val().is_nan())))
        print('conv10 : %d ' % (sum(self.conv10.debug_val().is_nan())))
        print('conv11 : %d ' % (sum(self.conv11.debug_val().is_nan())))
        print('conv12 : %d ' % (sum(self.conv12.debug_val().is_nan())))
        print('conv13 : %d ' % (sum(self.conv13.debug_val().is_nan())))
        print('conv14 : %d ' % (sum(self.conv14.debug_val().is_nan())))
        print('conv15 : %d ' % (sum(self.conv15.debug_val().is_nan())))
        print('conv16 : %d ' % (sum(self.conv16.debug_val().is_nan())))
        print('conv17 : %d ' % (sum(self.conv17.debug_val().is_nan())))
        """
