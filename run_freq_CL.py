...
class Run(object):
    ...
    def train(self):
        ...
        print("********************Curriculum Learning (Frequency Mode)*******************")
        start_time = time.time()
        args.severity = 1
        ...
        for i in range(1, 5):
            ...
            for epoch in range(10):
                ...
                avg_g_loss = 0.0
                avg_d_loss = 0.0
                for batch_idx, real_images in enumerate(self.data_loader): #real_image: B x 3 x H x W
                    ...
                    binary_mask = np.zeros((1, height, width))
                    for j in range(i):
                        binary_mask = np.logical_or(binary_mask, binary_masks[j])
                    binary_mask = torch.FloatTensor(binary_mask.astype(np.float32))
                    ...
                    # This code block is used only for saving sample masked images. Use --save_fig flag.
                    if (args.save_fig and batch_idx < 20):
                        save_image(masked_images, "sample_%d_s%d.png" % (batch_idx, i), nrow=8, normalize=True)
                        if (batch_idx % 5 == 4):
                            i += 1
                            if (i == 5):
                                assert(False)
                    ...
                    if (batch_idx % 1000 == 0):
                        print('[%d/%d] Train Loss Sample (G/D): %.8f / %.7f' % (batch_idx, iters_per_epoch, g_loss.item(), d_loss.item()))
                        if (batch_idx == 0):
                            print('Elapsed time per iter: %.2f secs' % (time.time() - start_time))
                ...
                print('Train Loss Average (G/D): %.8f / %.7f' % (avg_g_loss, avg_d_loss))
                if (epoch == 0 and i == 0):
                    print('Elpased timer per epoch: %.2f secs' %  (time.time() - start_time))
