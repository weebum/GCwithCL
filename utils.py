...
def l2_norm(x):
    x = x**2
    y = torch.sqrt(reduce_sum(x) + epsilon)
    return y
...
def brush_stroke_mask(FLAGS,name='mask', severity=1):
    """
    min_num_vertex = 4
    max_num_vertex = 12
    ...
    log_severity = math.log(severity + 1, 2)

    min_num_vertex = 4 * severity
    max_num_vertex = 6 * severity
    mean_angle = 2*math.pi/5
    angle_range = 2*math.pi/15
    min_width = int(8 * log_severity)
    max_width = int(10 * log_severity)


    def generate_mask(H,W):
        average_radius = math.sqrt(H*H+W*W + epsilon) /8
        mask = Image.new('L',(W,H),0)

        for _ in range(2):#np.random.randint(1,4)):
